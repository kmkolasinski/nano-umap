...
class NanoUMAPV4(core.NanoUMAPBase):
    ...
    def fit_transform(self, dataset: np.ndarray) -> np.ndarray:
        ...
        weights = 1 / knn_min_scores[graph.row] ** 2
        epochs_per_sample = core.make_epochs_per_sample(weights, n_epochs)
        ...
        for epoch in pbar(range(n_epochs)):
            update_step_fn(
                ...
                n_neg_samples=self._negative_sample_rate,
